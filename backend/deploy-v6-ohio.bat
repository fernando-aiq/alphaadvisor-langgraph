@echo off
REM ============================================================================
REM Script de Deploy para AWS Elastic Beanstalk - AlphaAdvisor
REM Environment: Alphaadvisor-v6-env
REM Region: us-east-2 (Ohio)
REM ============================================================================

setlocal enabledelayedexpansion

REM Garantir que o script roda dentro da pasta backend
set "SCRIPT_DIR=%~dp0"
cd /d "%SCRIPT_DIR%"
...
echo ========================================
echo   Deploy para Elastic Beanstalk - AlphaAdvisor
...
if "%APP_EXISTS%"=="%APP_NAME%" (
    ...
    aws elasticbeanstalk create-application --application-name %APP_NAME% --description "AlphaAdvisor - Assistente Virtual de Investimentos" --region %REGION% >nul 2>&1
    if errorlevel 1 (
...
if "%ENV_STATUS%"=="" (
    echo   ERRO: Environment %ENV_NAME% nao encontrado na regiao %REGION%!
    echo   Crie o environment primeiro via AWS Console ou EB CLI
    exit /b 1
)
...
echo exclude_patterns = ['*.zip', 'data', '.git', 'venv', 'env', '.env', '*_DEPLOY*.txt', '*_DEPLOY*.md', 'deploy*.ps1', 'deploy*.bat', '__pycache__', '*.pyc', 'test_*.py', 'start_*.bat', 'RESTART_*.bat', '.elasticbeanstalk', 'gerar-zip*.py', 'criar-zip*.py', 'aguardar*.py', 'verificar*.py', 'create_zip_temp.py'] >> create_zip_temp.py
echo exclude_dirs = {'data', '.git', 'venv', 'env', '__pycache__', '.elasticbeanstalk'} >> create_zip_temp.py
echo. >> create_zip_temp.py
